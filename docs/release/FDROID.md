...
Optional env **`FDROID_GITLAB_STAGE`:** `push` | `mr` | `both` (default **`both`** when unset — e.g. local runs). In **GitHub Actions**, `release-fdroid-app.yml` sets this on the **job** `env` from **`gitlab_stage`** (**`push_for_ci`** → `push`, **`open_draft_mr`** → `mr`, **`push_and_open_draft_mr`** → `both`). The **`fdroid-metadata-mr`** composite checks out **`target_ref`** for app sources but then **replaces only** `tools/fdroid/publish_fdroid_mr.py` from the repository **default branch**, so old release tags still honor **`mr`** (no GitLab commit) and newer commit API behavior; **`metadata_static.yml`** / **`build_template.yml`** remain from the tag. **`target_ref`** may be left empty to resolve the **latest tag** on the default branch (see [WORKFLOWS](../development/WORKFLOWS.md#release-fdroid-app)).
