...
- **`publish_fdroid_gitlab_branch.py`** strips those keys from existing YAML before pushing to your fork branch (if they were left from older edits) so F-Droid can take descriptions from the GitHub repo. The script also checks for Fastlane **`en-US`** `short_description.txt` and `full_description.txt` (path overridable with **`FDROID_FASTLANE_METADATA_REL_PATH`**).
...
6. Normalizes **MaintainerNotes** from the repo file **`FDROID_MAINTAINER_NOTES_PATH`** (keep it aligned with the block in **`metadata_static.yml`** — see the comment at the top of that file).
7. Pushes the full YAML to **`FDROID_GITLAB_BRANCH`** on your fork (fixed name, **no version in the branch**). New fork branches are created from **`FDROID_GITLAB_FORK_PARENT_REF`** when the target branch does not exist yet.
8. Prints the **GitLab tree URL** and a **compare link** (defaults in CI: compare vs **`FDROID_GITLAB_COMPARE_BASE_REF`** when set, otherwise vs **`FDROID_GITLAB_FORK_PARENT_REF`**). It does **not** create an upstream merge request — you do that in GitLab when ready.

Optional GitHub **variables** (all forwarded from **[`release-fdroid-app.yml`](../../.github/workflows/release-fdroid-app.yml)**; empty → composite **[`publish-fdroid-gitlab-metadata`](../../.github/actions/publish-fdroid-gitlab-metadata/action.yml)** defaults): **`FDROID_METADATA_PATH`**, **`FDROID_GITLAB_BRANCH`**, **`FDROID_GIT_COMMIT_SUBJECT_PREFIX`**, **`FDROID_MAINTAINER_NOTES_PATH`**, **`FDROID_GITLAB_FORK_PARENT_REF`**, **`FDROID_GITLAB_COMPARE_BASE_REF`**. See [WORKFLOWS](../development/WORKFLOWS.md#secrets-and-variables). The workflow checks out **`target_ref`** for app sources but replaces only **`tools/fdroid/publish_fdroid_gitlab_branch.py`** from the repo **default branch** so old tags still run the current script; **`metadata_static.yml`** / **`build_template.yml`** / **`maintainer_notes.txt`** stay from the tag. **`target_ref`** may be left empty to use the **latest tag** on the default branch.
