...
4. Merges **three** per-ABI **Build** rows into `Builds` (replaces prior rows for the same pubspec build number, including when `commit` is unchanged; dedupes by `versionCode`).
