...
- Do **not** commit, push, tag, reset, or force-push without **explicit** user consent.
...
- CI / release: see [docs/development/WORKFLOWS.md](docs/development/WORKFLOWS.md) (shared **composite actions** under `.github/actions/`; **`test.yml`** runs on **PR** to `main` (and manual dispatch); **`version-tag-main`** on **push** to `main` runs **tests then** updates `pubspec` + tag; **F-Droid** (`release-fdroid-app.yml`) only opens the fdroiddata **MR** for a **ref you choose**; **Google Play / legacy** (`release-apk-aab-google-play.yml`) builds from an **existing tag** + **GitHub Release** — no extra commits/tags in those workflows).
