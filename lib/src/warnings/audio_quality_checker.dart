...
class AudioQualityChecker {
  ...
  static List<AudioWarning> checkTrack(Track track, AppLocalizations trans) {
      ...
      if (file.existsSync()) {
        ...
        if (sizeInMB > 100) {
          warnings.add(AudioWarning(
            ...
            message: trans.audioWarningFileSizeVeryLarge(sizeInMB.toStringAsFixed(1)),
          ...
          warnings.add(AudioWarning(
            type: WarningType.fileSize,
            message: trans.audioWarningFileSizeLarge(sizeInMB.toStringAsFixed(1)),
    ...
    if (track.duration.value.inMinutes > 10) {
      warnings.add(AudioWarning(
        ...
        message: trans.audioWarningDurationLong(track.duration.value.inMinutes.toString()),
        severity: WarningSeverity.medium,
      ...
      warnings.add(AudioWarning(
        type: WarningType.duration,
        message: trans.audioWarningDurationMedium(track.duration.value.inMinutes.toString()),
    ...
    if (track.sampleRate != null) {
      if (track.sampleRate! != 44100 && track.sampleRate! != 48000) {
        warnings.add(AudioWarning(
          ...
          message: trans.audioWarningSampleRateNonStandard(track.sampleRate.toString()),
          severity: WarningSeverity.low,
    ...
    if (track.audioSource != null) {
      ...
      if (track.audioSource!.index == 0) { // microphone
        warnings.add(AudioWarning(
  ...
  static Map<Track, List<AudioWarning>> checkAllTracks(List<Track> tracks, AppLocalizations trans) {
